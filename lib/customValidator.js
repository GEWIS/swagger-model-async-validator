...
CustomValidations.prototype.validate = async function(name, model, fieldName, value) {
    var errors = [];
    processErrors(errors, await checkCustomValidations(model.id || name, fieldName, value));
    processErrors(errors, await checkCustomValidationsFromModel(model, fieldName, value));
...
async function checkCustomValidations(name, fieldName, value) {
    ...
    if (validators) {
        await Promise.all(
            validators.map(async (validator) => {
                try {
                    await processErrors(errors, await validator(name, value));
                } catch (e) {
                    console.warn(e);
                }
            })
...
async function checkCustomValidationsFromModel(model, fieldName, value) {
    ...
    if (validators) {
        await Promise.all(
            validators.map(async (validator) => {
                try {
                    await processErrors(errors, await validator(fieldName, value));
                } catch (e) {
                    console.warn(e);
                }
            })
