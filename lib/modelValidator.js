...
function Validator(swagger) {
    ...
    if(swagger) {
        swagger.validateModel = async function(modelName, obj, allowBlankTarget, disallowExtraProperties) {
            ...
            return await validate(modelName, obj, model, models, allowBlankTarget, disallowExtraProperties, self.customValidators);
        };
        swagger.addFieldValidatorToModel = function(model, fieldName, validatorFunction) {
            return self.customValidators.addFieldValidatorToModel(model, fieldName, validatorFunction);
        };

        swagger.addFieldValidator = function(modelName, fieldName, validatorFunction){
            return self.customValidators.addFieldValidator(modelName, fieldName, validatorFunction);
        };
    }
}

Validator.prototype.validate = async function(target, swaggerModel, swaggerModels, allowBlankTarget, disallowExtraProperties) {
    return await validate('rootModel', target, swaggerModel, swaggerModels, allowBlankTarget, disallowExtraProperties);
};
...
async function validate(name, target, swaggerModel, swaggerModels, allowBlankTargets, disallowExtraProperties) {
    ...
    if(targetType !== "object" && targetType !== modelType) {
        var errors = await validateSpecification(name, target, swaggerModel, swaggerModels, allowBlankTargets, disallowExtraProperties);
        if(errors !== null) {
            return createReturnObject(new Error('Unable to validate a model with a type: ' + targetType + ', expected: ' + modelType));
        }
    }
    ...
    var validationErrs = await validateSpecification(name, target, model, swaggerModels, allowBlankTargets, disallowExtraProperties);
...
async function validateSpecification(name, target, model, models, allowBlankTargets, disallowExtraProperties) {
    var properties = model.properties;
    var errors = [];

    // if there is a $ref property, it's a ref to other model
    if(model.$ref) {
        var refErrors = await validateType(name, target, model, models, allowBlankTargets, disallowExtraProperties);
    ...
    // if there are no properties, it's a reference to a primitive type
        ...
        var singleValueErrors = await validateValue(name, model, target, models, allowBlankTargets, disallowExtraProperties);
        ...
        for (var key in properties) {
            ...
            if (value !== undefined && field['x-do-not-validate'] !== true) {
                if(value != null) {
                    //noinspection JSUnfilteredForInLoop
                    var valueErrors = await validateValue(key, field, value, models, allowBlankTargets, disallowExtraProperties);
                    ...
                    //noinspection JSUnfilteredForInLoop
                    var customErrors = await self.customValidators.validate(name, model, key, value);
...
async function validateValue(key, field, value, models, allowBlankTargets, disallowExtraProperties) {
    var errors = [];
    if(value !== undefined) {
        var typeErr = await validateType(key, value, field, models, allowBlankTargets, disallowExtraProperties);
        if (typeErr) {
...
async function validateType(name, value, field, models, allowBlankTargets, disallowExtraProperties) {
    ...
    if(!expectedType) {
        if(models && field.$ref) {
            ...
            {
                return await validateType(name, value, model, models, allowBlankTargets, disallowExtraProperties)
            }
            ...
            for(var x in oneOfArray) {
                ...
                var errorResult = await validateType(name, value, oneOfArray[x], models, allowBlankTargets, disallowExtraProperties);
            ...
            for(var x in anyOfArray) {
                ...
                var errorResult = await validateSpecification(name, value, anyOfArray[x], models, allowBlankTargets, disallowExtraProperties);
                if (errorResult && !errorResult.valid) {
            ...
            for(var x in allOfArray) {
                ...
                var errorResult = await validateType(name, value, allOfArray[x], models, allowBlankTargets, false);
                if (errorResult && !errorResult.valid) {
    ...
    if(expectedType === 'object') {
        return await validateObject(name, value, field, models, allowBlankTargets, disallowExtraProperties);
    }

    if(expectedType === 'array') {
        return await validateArray(name, value, field, models, allowBlankTargets, disallowExtraProperties);
    }
...
async function validateObject(name, value, field, models, allowBlankTargets, disallowExtraProperties) {
    if(typeof value != "object") {
        return new Error(name + ' is not a type of object. It is a type of ' + typeof value);
    }
    if(field && field.properties) {
        return await validate(name, value, field, models, allowBlankTargets, disallowExtraProperties);
    } else {
        return null;
    }
}

async function validateArray(name, value, field, models, allowBlankTargets, disallowExtraProperties) {
    ...
    if(field.items && field.items.type) {
        ...
        for (const value1 of value) {
            var errors1 = await validateValue(name + countItems.toString(), field.items, value1, models, allowBlankTargets, disallowExtraProperties);
            if(errors1) {
        ...
        if(model) {
            var arrayErrors2 = [];
            for (const value1 of value) {
                var errors2 = await validate(name, value1, model, models, allowBlankTargets, disallowExtraProperties);
        ...
        for (const value1 of value) {
            ...
            if (discriminator) {
                possibleTypes = [];
                var discriminatorValue = value1[discriminator.propertyName];

                if (mappedTypes[discriminatorValue]) {
                    var refErrors = await validate(name, value1, mappedTypes[discriminatorValue], models, allowBlankTargets, disallowExtraProperties);
            ...
            for (const possibleType of possibleTypes) {
                if(!passedValidation) {
                    if ((await validate(name, value1, possibleType, models, allowBlankTargets, disallowExtraProperties)).valid) {
                        passedValidation = true;
                    }
                }
            }
